<template>
<div class="" style="">
    		<galleryList :projects="projects" title="Research" hue="#7ebdbb" path="/research/projects" v-bind="{ 'msgChild': msgChild }"/>
...
export default {  
  ...
  asyncData(context) {
   ...
   //var matcher = new RegExp(/(${animalPath})\/.*md$/);
    ...
    var animalsCategory = require.context('~/content/research/', true, /\.*md$/);
    var activeAnimals=[]
   // console.log(animalsCategory.keys(),'leeeys',animalPath)
    const context2 = require.context('~/content/research/', true, /\.md$/);
  ...
  mounted() {
  	...
  	transition: {
		...
		beforeEnter:function(){
			console.log('projectsenterbefore')
		
			
		},
		enter:function(el){
			console.log('projectsenter')
		
		},
		leave:function(el){
console.log('projectleave')
    }
  }
