<template>
<div class="" style="">
    		<galleryList :projects="projects" :title="'Facilities'" path="/facility/facilities" v-bind="{ 'msgChild': msgChild }"/>
...
export default {  
  ...
  asyncData(context) {
   ...
   //var matcher = new RegExp(/(${animalPath})\/.*md$/);
    ...
    var animalsCategory = require.context('~/content/facility/', true, /\.*md$/);
    var activeAnimals=[]
   // console.log(animalsCategory.keys(),'leeeys',animalPath)
    const context2 = require.context('~/content/facility/', true, /\.md$/);
