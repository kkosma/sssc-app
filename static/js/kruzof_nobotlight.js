...
var OBJLoader2Example = (function () {
  ...
  OBJLoader2Example.prototype.initGL = function () {
   ...
   // this.scene.add(pointLight);
    ...
    this.skinLightBottom = new THREE.DirectionalLight(0xEDDABC, .6);
    this.skinLightBack = new THREE.DirectionalLight(0x66A0F2,.2);
    var skinLightTop = new THREE.DirectionalLight(0x66A0F2, .6);
...
/*
    ...
    var bump = THREE.ImageUtils.loadTexture("images/kruzof/normal8k.jpg");
    var lamberto = new THREE.MeshStandardMaterial({ color: 0xFFF4E3, roughness: .56, metalness: .2, })
    lamberto.normalMap=bump
    function MatcapMaterial(matcapImage) {
     
        if (matcapImage == null) {
          matcapImage = '/js/three/three-matcap-material/assets/LitSphere_test_05.jpg'
        } else if (matcapImage.indexOf("/") == -1) {
          matcapImage = `/js/three/three-matcap-material/assets/${matcapImage}.jpg`
        }
        //console.log(vs,fs,'shaders')
      var uniforms = 
 
        {
          ...
          tMatCap: {
            type: 't',
            value: THREE.ImageUtils.loadTexture('/js/three/three-matcap-material/assets/al_ancient_bone.jpg')
          },
          tNormal: { type: 't', value: THREE.ImageUtils.loadTexture("images/kruzof/normal8k.jpg") },
          
          //useNormal: { type: 'f', value: 1.0 },
          useRim: { type: 'f', value: 1 },
          rimPower: { type: 'f', value: 5 },
    ...
    var matcap = new MatcapMaterial()
    matcap.extensions.derivatives = true
    matcap.normalMap=bump
    //matcap.uniforms.tNormal.value.wrapS = matcap.uniforms.tNormal.value.wrapT =
...
/*
      ...
      matcap.uniforms.tNormal.value = bump
      matcap.uniforms.tNormal.value.needsUpdate = true;
      matcap.uniforms.tNormal.value.wrapS = matcap.uniforms.tNormal.value.wrapT = THREE.RepeatWrapping;
      matcap.uniforms.useNormal.value = 1.;

    }
    //adjustNormalMap()
    //matcap.normalMap = bump
    console.log(matcap,'matcap')
    ...
    objLoader.load('images/kruzof/kruzof-250-bakeuv4.obj', callbackOnLoad, loadProgress, null, null, true);
  ...
  // Load Skin
    ...
    this.endGame = function () {
      ...
      var tweenSkinPos = function () {
        ...
        deltaBonesZ = (app.orca.position.z +.65) - initialBonesZ
